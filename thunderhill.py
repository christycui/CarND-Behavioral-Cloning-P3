  ...
  while 1: # Loop forever so the generator never terminates
    ...
    for offset in range(0, num_samples, batch_size):
      ...
      for batch_sample in batch_samples:
        ...
        single_input = input_all.append([center_image,speed,CTE])
      ...
      X_train = np.concatenate((X_train, images_flipped), axis=0).tolist()
      y_train = np.concatenate((y_train, angles_flipped), axis=0).tolist()
      print(X_train)
...
input_cte = Input(shape=(1,),name='input_cte')

output = merge([branch_image, input_speed, input_cte], mode='concat', concat_axis=1)
output = Dense(2)(output)

model =  Model(input = [input_image, input_speed, input_cte], output = [output])
